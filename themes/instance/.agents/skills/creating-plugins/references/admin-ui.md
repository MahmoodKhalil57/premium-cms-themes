...
import { usePluginAPI } from "@premium-cms/admin";
...
Pre-built components from `@premium-cms/admin`:

```typescript
import { Card, Button, Input, Select, Toggle, Table, Loading, Alert } from "@premium-cms/admin";
```
...
export default {
	...
	format: "esm",
	dts: true,
	external: ["react", "react-dom", "@premium-cms/emdash", "@premium-cms/admin"],
};
```

Keep React and `@premium-cms/admin` as externals to avoid bundling duplicates.
